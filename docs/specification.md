...
3. メッセージ機能 
    1. メッセージの種類 
        ...
        - 教員→学生DM: 教員が特定の学生を指定して送信(指定した学生と全教員のみに届く)
        - 学生同士のDM: 不可 
        - 教員同士のDM: 不可 
    2. メッセージの制約 
...
6. チャットルーム管理 
    ...
    2. チャットルームの作成 
        ...
        - 作成時に招待リンクが生成される
        ### ルームIDおよび招待リンク
        - ルーム作成者が任意で決めるか、自動で生成するかを選ぶ
        - 任意で決める場合、4~16文字の英数字のみが許可される
        - 自動生成の場合、8文字の小文字英数字で生成される
        - 既出のIDを使うことはできない(ルームが削除された場合は使用できる)
        - 招待リンクは`https://[domain]/room/[roomId]`の形になる
    3. チャットルームへの参加 
        - 招待リンクからの参加のみ 
    ...
    5. チャットセッションの管理 
        ...
        ### セッションの終了 
        - 教員が「履歴クリア」ボタンを押すことでチャットをまっさらな状態にする 
